{{-- Spotify + Instagram widget kartları (Wi-Fi yalnızca üst barda) --}}
@if($spotifyUrl !== '' || $instagramUrl !== '')
@php
    $socialCardCount = ($spotifyUrl !== '' ? 1 : 0) + ($instagramUrl !== '' ? 1 : 0);
@endphp
<section
    class="menu-social-grid mb-5 grid w-full max-w-md grid-cols-2 gap-3 px-5 mx-auto {{ $socialCardCount === 1 ? 'max-w-xs' : '' }}"
...
>
    ...
    <article class="menu-social-card flex aspect-square min-w-0 flex-col justify-between overflow-hidden rounded-2xl border border-white/5 bg-[#262220]/50 p-4 backdrop-blur-md {{ $socialCardCount === 1 ? 'col-span-2' : '' }}">
        <div class="flex h-10 w-10 shrink-0 items-center justify-center rounded-full border border-[#1DB954]/25 bg-[#1DB954]/15" aria-hidden="true">
            <svg class="h-6 w-6 text-[#1DB954]" viewBox="0 0 24 24" fill="currentColor" role="img" aria-label="Spotify">
    ...
    <a
        ...
        target="_blank"
        rel="noopener noreferrer"
        class="menu-social-card flex aspect-square min-w-0 flex-col justify-between overflow-hidden rounded-2xl border border-white/5 bg-[#262220]/50 p-4 backdrop-blur-md transition hover:border-[#E67E22]/25 hover:bg-[#262220]/70 {{ $socialCardCount === 1 ? 'col-span-2' : '' }}"
    >
        <div class="flex h-10 w-10 shrink-0 items-center justify-center rounded-full border border-[#E67E22]/20 bg-gradient-to-br from-[#f9ce34]/20 via-[#ee2a7b]/20 to-[#6228d7]/20" aria-hidden="true">
            <svg class="h-6 w-6 text-[#E67E22]" viewBox="0 0 24 24" fill="currentColor" role="img" aria-label="Instagram">
